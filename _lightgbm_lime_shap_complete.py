...
df_valid['Forecast_LightGBM'] = model.predict(X_test)

overall_data = pd.concat([df_train, df_valid], sort=False)

overall_data[['Weighted_Price','Forecast_LightGBM']].plot(figsize=(15, 5))

df_valid[['Weighted_Price','Forecast_LightGBM']].plot(figsize=(15, 5))
...
test_mae = mean_absolute_error(df_valid['Weighted_Price'], df_valid['Forecast_LightGBM'])
test_rmse = np.sqrt(mean_squared_error(df_valid['Weighted_Price'], df_valid['Forecast_LightGBM']))
test_r2 = r2_score(df_valid['Weighted_Price'], df_valid['Forecast_LightGBM'])
...
model = lgbm.LGBMRegressor().fit(X_train.values, y_train)
